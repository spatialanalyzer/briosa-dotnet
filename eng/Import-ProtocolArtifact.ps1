...
try {
    ...
    Assert-Equal $manifest.schema_version 1 "Unsupported protocol manifest schema."
    Assert-Equal $manifest.artifact_kind "briosa_protocol" "Unexpected artifact kind."
    Assert-Equal $manifest.client_generation_contract_version 1 "Unsupported client generation contract."
    ...
    if (-not $Update) {
        ...
        Assert-Equal $lock.schema_version 1 "Unsupported protocol lock schema."
        ...
        Assert-Equal $manifest.catalog_id $lock.catalog.id "Catalog ID drift."
        Assert-Equal $manifest.catalog_revision $lock.catalog.revision "Catalog revision drift."
        Assert-Equal $manifest.spatial_analyzer_target $lock.catalog.spatial_analyzer_target "SA target drift."
        Assert-Equal $manifest.target_protocol_package $lock.protocol.target_package "Target package drift."
        Assert-Equal $manifest.core_protocol_package $lock.protocol.core_package "Core package drift."
        Assert-Equal $manifest.conformance_fixture_sha256 $lock.conformance.sha256 "Conformance fixture drift."
    }
...
public static class BriosaProtocolIdentity
{
    ...
    public const string CoreProtocolPackage = "$($manifest.core_protocol_package)";
    /// <summary>Gets the exact-SpatialAnalyzer-target protobuf package.</summary>
    public const string TargetProtocolPackage = "$($manifest.target_protocol_package)";
    ...
    public const string CatalogId = "$($manifest.catalog_id)";
    /// <summary>Gets the reviewed command-catalog revision.</summary>
    public const string CatalogRevision = "$($manifest.catalog_revision)";
    /// <summary>Gets the aggregate shared-conformance-fixture fingerprint.</summary>
    public const string ConformanceFixtureSha256 = "$($manifest.conformance_fixture_sha256)";
}
"@
    ...
    if ($Update) {
        $lock = [ordered]@{
            ...
            protocol = [ordered]@{
                ...
                core_package = [string]$manifest.core_protocol_package
                target_package = [string]$manifest.target_protocol_package
            }
            catalog = [ordered]@{
                id = [string]$manifest.catalog_id
                revision = [string]$manifest.catalog_revision
                spatial_analyzer_target = [string]$manifest.spatial_analyzer_target
                coverage_sha256 = [string]$manifest.catalog_coverage_sha256
            }
            conformance = [ordered]@{
                sha256 = [string]$manifest.conformance_fixture_sha256
