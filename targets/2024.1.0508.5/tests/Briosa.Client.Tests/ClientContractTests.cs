...
public sealed partial class ClientContractTests
{
    ...
    public async Task RelationshipInputsUseItemNames()
    {
        ...
        await client.DeleteRelationshipAsync(new CollectionItemName
        {
            CollectionName = "Inspection",
            ItemName = "R1",
            ItemType = ItemType.Relationship,
        });
        var request = Assert.IsType<Transport.DeleteRelationshipRequest>(transport.LastOperationRequest);
        Assert.Equal("R1", request.RelationshipName.ItemName);
        Assert.Equal(Transport.ItemType.Relationship, request.RelationshipName.ItemType);
    }
    ...
    {
        Assert.Equal(
            "briosa-protocol-0.7.0-preview.2-sa-2024.1.0508.5",
    ...
    {
        Assert.Equal(462, BriosaClient.WaveAOperationNames.Count);
        Assert.Equal(462, BriosaClient.WaveAOperationNames.Distinct().Count());
    ...
    public void WaveBContractPublishesEveryRpcInTheExtendedServices()
    {
        ...
        Assert.Equal(586, operationNames.Length);
        Assert.Equal(586, operationNames.Distinct().Count());
